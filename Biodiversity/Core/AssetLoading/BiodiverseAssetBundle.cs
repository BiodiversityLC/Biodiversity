...
{
    /// <summary>
    /// Tells if the assets are loaded. This will always be true unless there was
    /// an error during the assets loading phase, or if the assets are optional
    /// and thus not loaded if not installed.
    /// </summary>
    ...
    /// <param name="isOptional">If false (default), assets are part of the main Biodiversity assets and should always be loaded.
    /// If true, assets are optional/part of an add-on, are located in an external mod folder and can be missing if not installed.</param>
    ...
    protected BiodiverseAssetBundle(string filePath, bool isOptional = false)
    {
        AssetBundle bundle = BiodiversityPlugin.LoadBundle(filePath, isOptional);
        if (bundle == null)
        {
            if (!isOptional)
            {
