...
public abstract class BiodiverseAI : EnemyAI
{
    ...
    protected bool GetPlayersCloseBy(float radius, out List<PlayerControllerB> players)
    {
        ...
        if (Physics.OverlapSphereNonAlloc(base.transform.position, radius, hitColliders, 8, QueryTriggerInteraction.Ignore) <= 0)
        { // no clue what the 8 means but it was in the thumpers code lol
            players = [];
            return false;
        }
