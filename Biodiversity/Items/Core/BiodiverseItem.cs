...
public class BiodiverseItem : PhysicsProp
{
    ...
    /// A string identifier representing the type/category of the audio clip to be played (e.g., "Stun", "Laugh", "Ambient").
    /// </param>
    ...
    /// Indicates whether the played audio should be transmitted through the walkie-talkie system.
    /// </param>
    /// <param name="audibleByEnemies">
    /// Determines whether the played audio should be detectable by <see cref="EnemyAI"/>.
    /// </param>
    /// <param name="slightlyVaryPitch">
    /// Whether to slightly vary the pitch between <c>0.9</c> and <c>1.1</c> randomly.
    /// </param>
    [ServerRpc]
        ...
        bool slightlyVaryPitch = false)
    {
        ...
        {
            LogWarning($"Audio Clip Type '{audioClipType}' not found, is null, or empty on server.");
            return;
        }
        ...
        {
            LogWarning($"Audio Source Type '{audioSourceType}' not found or null on server.");
            return;
        }
    ...
    private void PlayAudioClipTypeClientRpc(
        ...
        bool audibleByEnemies = false,
        bool slightlyVaryPitch = false) =>
        PlayAudioClipType(audioClipType, audioSourceType, clipIndex, interrupt, audibleInWalkieTalkie, audibleByEnemies, slightlyVaryPitch);

    /// <summary>
    /// Plays the selected audio clip on the specified <see cref="UnityEngine.AudioSource"/>.
    /// </summary>
    /// <param name="audioClipType">
    /// A string identifier representing the type/category of the audio clip to be played (e.g., "Stun", "Laugh", "Ambient").
    /// </param>
    ...
    /// Whether to slightly vary the pitch between <c>0.9</c> and <c>1.1</c> randomly.
    /// </param>
    protected void PlayAudioClipType(
        string audioClipType,
        string audioSourceType,
        int clipIndex,
        bool interrupt = false,
        ...
        bool slightlyVaryPitch = false)
    {
        ...
        {
            LogWarning($"Invalid clip index {clipIndex} received for type '{audioClipType}' (Count: {clipArr.Length}).");
            return;
        }
        ...
        if (!clipToPlay)
        {
            LogWarning($"Audio clip at index {clipIndex} for type '{audioClipType}' is null.");
            return;
        }

        if (!AudioSources.TryGetValue(audioSourceType, out AudioSource selectedAudioSource) || !selectedAudioSource)
        {
            LogWarning($"Audio Source Type '{audioSourceType}' not found or is null.");
            return;
        }

        LogVerbose(
            $"Playing audio clip: {clipToPlay.name} for type '{audioClipType}' on AudioSource '{audioSourceType}'.");
